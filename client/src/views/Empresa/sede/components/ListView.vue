          ...
          <tr v-for="element in queryset" :key="element.id">
            ...
            <td>
              <div class="btn-group" role="group" aria-label="Basic example">
                <RouterLink
                    :to="{ name: 'sedes-detail', params: { nit: url.currentRoute.value.params.nit, id: element.id } }"
                    type="button" class="btn btn-info">
                  Detalles <i class="bi bi-search"></i>
                </RouterLink>
                <RouterLink
                    :to="{ name: 'sedes-update', params: { nit: url.currentRoute.value.params.nit, id: element.id } }"
...
const getData = async () => {
  loading.value = true
  await getSedes(url.currentRoute.value.params.nit)
      .then((Response) => {
        queryset.value = Response.data
      })
      .finally(() => {
