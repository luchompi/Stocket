<template>
    <div class="input-group mb-3">
        ...
        <input type="text" class="form-control" placeholder="Username" aria-label="Username" aria-describedby="inputSearch"
            v-model="search" />
    </div>
    <table class="table table-striped table-hover">
        ...
        <tbody>
            <tr v-for="(item) in queryset" :key="item.PID">
                ...
                <td>{{ item.estado }}</td>
                <td>
                  <RouterLink :to="{name:'fix-details',params:{id:item.PID}}" type="button" class="btn btn-primary">Ver <i class="bi bi-search"></i></RouterLink>
                </td>
            </tr>
        </tbody>
    </table>
...
import { obtenerMantenimientos } from '../services/mantenimiento.services';
...
watchEffect(() => {
    search.value.length ? searchMaintenance() : getMaintenance()
})
