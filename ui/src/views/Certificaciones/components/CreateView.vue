...
<template>
  ...
  <ul class="list-group" v-if="employee.length" v-for="item in employee">
    <li class="list-group-item" @click="storeEmployee(item)">{{ item.iden }} - {{ item.first_name }} {{
        item.last_name
      }}
    </li>
  </ul>
  <br>
  <div class="alert alert-primary" role="alert" v-if="store.iden">
    <strong>{{ store.iden }}</strong> {{ store.first_name }} {{ store.last_name }} Seleccionado
    <button type="button" class="btn btn-success" @click="clearStore">Consultar otro</button>
  </div>
  <br>
  <div class="container">
